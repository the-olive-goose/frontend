...
/**
 * A testimonial pinned to one candle, shown on that candle's product page.
 *
 * Same shape and the same editor as the homepage carousel above it, on purpose —
 * these are curated by the shop from real submissions, not a separate reviews
 * system that collects and publishes on its own. The only addition is which
 * product it belongs to.
 *
 * `product_id` matches the id in the products content blob (a short string like
 * "1"). A quote whose product no longer exists simply stops being rendered
 * rather than erroring — deleting a candle must not break the editor.
 */
export interface ProductTestimonial extends Testimonial {
  product_id: string;
}
...
export interface TestimonialsContent {
  ...
  headline: string;
  items: Testimonial[];
  /** Heading above the quotes on a product page. */
  product_headline: string;
  /**
   * Curated quotes shown on individual product pages, edited in the same place
   * as the homepage carousel (Admin → Home Page → Testimonials → Product Page
   * Testimonials). Absent on content saved before this existed — mergeSection
   * fills it from the default, so it is safe to read without a guard.
   */
  product_items: ProductTestimonial[];
}
...
// ── Product feed ───────────────────────────────────────────────────────────────
// Settings for the RSS feed at /feed.xml that Google Merchant Center and Meta
// Commerce Manager read to build their catalogues. The document itself is
// generated by the API (backend/productFeed.js) so a price edited in admin
// reaches the ad platforms without a redeploy; this is only what an admin
// chooses. Shipping is deliberately absent — it comes from Pickup & Delivery so
// the feed can never quote a rate the shipping page contradicts.

export interface ProductFeedContent {
  /** Serve the feed at all. Off means /feed.xml returns 404. */
  enabled: boolean;
  /** Blank falls back to the SEO site name. */
  brand: string;
  /** Google's `condition` attribute. Handmade stock is always "new". */
  condition: "new" | "refurbished" | "used";
  /** Google product taxonomy — either the full path or its numeric id. */
  google_product_category: string;
  /** Your own grouping, shown in reports and usable for ad segmentation. */
  product_type: string;
  /**
   * Whether these products carry a GTIN/barcode. Handmade candles do not, and
   * saying so explicitly is what keeps Google from disapproving the whole feed
   * for missing identifiers — leave this off.
   */
  identifier_exists: boolean;
  /** Send sold-out products as out_of_stock rather than dropping them. */
  include_out_of_stock: boolean;
  /** Include gallery images as additional_image_link (up to 10 per product). */
  include_gallery_images: boolean;
  /** Two-letter country the shipping rate applies to. */
  shipping_country: string;
  /**
   * Working days between an order arriving and the parcel leaving — the making
   * time, for a shop that pours each candle after it is ordered.
   *
   * This is how a made-to-order shop is correctly described to Google, and it is
   * not cosmetic. Google shows an estimated delivery date in the ad itself; with
   * no handling time it assumes the parcel goes out the same or next day and
   * advertises a date that cannot be met. That is a bad customer experience
   * before the click and a policy problem after it, because Google checks
   * promised dates against reality.
   *
   * `availability` stays `in_stock` throughout, which is correct: the shop can
   * accept the order. `preorder` and `backorder` mean something else — an item
   * that does not exist yet, or one that has run out — and using either for
   * made-to-order suppresses the listing for no reason.
   *
   * Zero for both means "no handling time declared", and the fields are omitted.
   */
  min_handling_time: number;
  max_handling_time: number;
  /** Free-text label for splitting the catalogue in ad campaigns. */
  custom_label_0: string;
}

export const DEFAULT_PRODUCT_FEED: ProductFeedContent = {
  enabled: false,
  brand: "",
  condition: "new",
  ...
  shipping_country: "IE",
  // Two to four working days to pour, cure and pack. A starting point the shop
  // can correct — better a stated figure than none, which Google reads as
  // same-day dispatch.
  min_handling_time: 2,
  max_handling_time: 4,
  custom_label_0: "",
...
export interface SiteContent {
  ...
  productFeed: ProductFeedContent;
...
export const DEFAULT_CONTENT: SiteContent = {
  ...
  testimonials: {
    ...
    product_headline: "What people say about this one",
    // Empty by default: a product page shows this section only once the shop has
    // put a real quote against that candle. Seeding it with invented praise
    // would put made-up reviews on a live shop.
    product_items: [],
  },
  ...
  productFeed: DEFAULT_PRODUCT_FEED,
